...
### Added & Improved
- **Завершение экрана «Автоматизации» и конструктора правил (`BL-105`, `86eyr606z`)**:
  - Доска получила поиск по названию/действию/метрике, фильтр действий, точный счётчик результатов и отдельные loading, empty, filtered-empty и retryable error states.
  - Карточки правил, группы и выбор шаблона доступны с клавиатуры; модальные окна удерживают фокус и возвращают его в исходный control после закрытия.
  - Опасные STOP/START/изменения бюджета требуют контекстного подтверждения; удаление показывает влияние на связанные кабинеты, а create/edit/group/bulk mutations защищены от повторной отправки и недоступны при неполных данных.
  - Длинные названия правил, групп и кабинетов получили управляемый перенос; на mobile доска и экран правила складываются вертикально без document-level horizontal overflow.
  - Frontend contract расширен проверками устойчивых состояний, подтверждений, focus management и responsive-контракта без изменений backend API.
