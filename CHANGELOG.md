...
### Added & Improved
- **Завершено рабочее пространство «История действий» (`86eyrx9c6`)**:
  - Server-backed поиск теперь охватывает названия, сообщения, типы событий и технические ID; фильтры категории, статуса, кабинета и периода получили сброс и отдельное состояние «ничего не найдено».
  - Desktop-таблица и mobile-лента показывают одинаковые статусы, доступность undo и полные детали события; длинные тексты, идентификаторы и JSON остаются внутри локальных контейнеров без document-level overflow.
  - Добавлены loading skeleton, empty, error/retry и live-region состояния, а version guard не позволяет устаревшему ответу другого фильтра или workspace перерисовать экран.
  - Undo получил предварительное объяснение, подтверждение, inline-ошибку и точную серверную причину недоступности; modal возвращает клавиатурный фокус к открывшему элементу.
  - API нормализует выход за диапазон пагинации и считает отменённые события отдельно от выполненных, сохраняя строгую изоляцию активного workspace.
