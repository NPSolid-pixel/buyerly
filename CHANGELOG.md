...
### Security
- **Удаление хардкода API-ключей и усиление маскирования логов (`[86eyr5v9p]`)**:
  - Из скрипта деплоя `scripts/deploy.sh` полностью удален жестко зашитый Resend API ключ; скрипт корректно сохраняет существующие конфигурации `.env` без переопределения секретов.
  - В модуле логирования `core/logging_config.py` расширены правила маскирования (`_SECRET_PATTERNS`): добавлены регулярные выражения для скрытия ключей Resend (`re_...`) и полей паролей (`password=...`).
  - Добавлены соответствующие модульные тесты в `tests/test_logging.py`.
