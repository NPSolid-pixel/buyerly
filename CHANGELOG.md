...
### Added & Improved
- **Завершённое рабочее пространство «Эффективность» (`86eyr6070`)**:
  - KPI дополнены честным сравнением с предыдущим сохранённым снимком и доступной воронкой текущего периода без новых или несовместимых API.
  - Добавлены явные loading, empty, error, cached-refresh и partial-data состояния с полезными действиями и сохранением уже загруженных данных при ошибке обновления.
  - Фильтры периода, представления и статуса, сортировка и настройка колонок получили доступные выбранные состояния, клавиатурную навигацию и live-status обратную связь.
  - Desktop-таблица сохраняет локальную горизонтальную прокрутку, а мобильные карточки, KPI, сравнения, длинные названия, заметки и денежные значения не создают document-level overflow на 390/768/1024/1440px.
  - Добавлен отдельный статический контракт `test_efficiency_workspace_contract.py` для состояний экрана, существующих API, сравнения/воронки, доступности и responsive containment.
