...
Production checkout `/opt/buyerly` обязан принадлежать тому же системному пользователю, от которого запускается deploy, а `origin` — указывать на канонический `hiurano/buyerly`. Несовпадение владельца или remote останавливает выкладку до любых изменений. В production-образ входят только runtime-каталоги; тесты, документация, локальные диагностические скрипты, транскрипты и исследовательские снимки интерфейсов не копируются.
