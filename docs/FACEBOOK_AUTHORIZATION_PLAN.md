...
- `Переподключить`;
- `Отключить от Buyerly` — удаляет зашифрованный token и безопасно выключает связанные кабинеты/правила, но не меняет разрешения приложения в Meta;
- отдельное `Отозвать разрешения в Meta` — требует усиленного подтверждения, предупреждает о влиянии на тот же Facebook-профиль в других workspace и удаляет подключение только после подтверждённого ответа Meta.

Таблица подключений агрегирует кабинеты, Business Manager и расход строго по `meta_connection_id`. Состояния `active`, `expiring`, `expired`, `missing_scopes` и `needs_reconnect` имеют текстовые labels, описание следующего действия и время последней проверки; цвет используется только как дополнительный сигнал.
