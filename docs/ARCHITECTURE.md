...
flowchart LR
    ...
    R --> B
    A --> E[Resend API · Транзакционная почта]
```

Продакшен состоит из независимых сервисов:

- `web` отдаёт интерфейс и направляет `/api/*` и `/health/*` в API;
- `api` отвечает за авторизацию, профиль, воркспейсы, инвайты, онбординг, кабинеты, правила, группы правил, сводки и аудит;
...
- `redis` атомарно хранит общие sliding-window лимиты для всех экземпляров API;
- `migrate` — единственный production-путь изменения схемы PostgreSQL: последовательный `alembic upgrade head` под advisory lock, затем проверка версии и обязательных таблиц/колонок;
- `Resend` — внешний транзакционный почтовый сервис для мгновенной доставки OTP-кодов входа и приглашений в команду.

Такое разделение не даёт сбою Telegram polling или фоновой проверки уронить веб-интерфейс. API можно проверять отдельно через `/health/live` и `/health/ready`; readiness включает PostgreSQL и обязательный production Redis.

## Данные и мультитенантность

SQLAlchemy-модели находятся в `database/models.py`:

- `User` — пользователь, персональные данные (`first_name`, `last_name`, `email`, `avatar_url`), статус онбординга, активный воркспейс и доступ;
- `Workspace` — изолированное рабочее пространство компании/команды со слагом (`slug`), бейджем, цветом и логотипом;
- `WorkspaceMember` — членство пользователя в воркспейсе с гранулярной ролью (`owner`, `admin`, `buyer`, `viewer`);
- `WorkspaceInvite` — приглашение в команду (персональное по email или публичная ссылка) со сроком жизни и лимитом использований;
- `EmailVerificationCode` — назначение, scope, HMAC-хэш и состояние доставки 6-значного OTP с защитой от брутфорса и окном валидности 15 минут; активный код уникален внутри scope;
- `Account` — Meta-кабинет, привязка к воркспейсу (`workspace_id`), владелец, статус, источник подключения, собственное название, рабочая заметка и назначенные правила;
- `AccountGroup` и `AccountGroupMember` — собственная именованная many-to-many группировка кабинетов для аналитических срезов внутри воркспейса;
- `RulePreset` — исполняемое правило воркспейса;
- `RuleGroup` и `RuleGroupItem` — переиспользуемая упорядоченная группа правил воркспейса;
...
- `AuditEvent` — неизменяемая история действий продукта с привязкой к `workspace_id`;
...
Владение бизнес-данными и видимость изолированы по `Workspace.id` (`workspace_id`). Пользователь получает доступ к кабинетам и правилам только через активный воркспейс, участником которого он является (`WorkspaceMember`). Поле `owner_user_id` определяет создателя/владельца объекта. Смена Telegram ID или переключение между воркспейсами сохраняет полную изоляцию данных между командами.
...
## Границы доступа и RBAC

Внутри каждого воркспейса применяется ролевая модель доступа:
- `owner` (Владелец): полный доступ, удаление воркспейса, передача владения (`transfer-ownership`).
- `admin` (Администратор): управление участниками, создание инвайтов, настройка интеграций, правил и кабинетов.
- `buyer` (Байер): управление кабинетами, запуск правил, просмотр аналитики и отмена действий (Undo).
- `viewer` (Наблюдатель): доступ только для чтения аналитики и сводок без права изменения настроек.

Веб принимает подписанные данные Telegram (`Authorization: tma <initData>`), постоянный токен входа (`Authorization: Bearer <token>`) или заголовок `X-Auth-Token`. Секреты Meta и Telegram удаляются из журналов форматтером до записи на диск. Детальное описание концепций и эндпоинтов приведено в [WORKSPACES_AUTH_AND_INVITES.md](WORKSPACES_AUTH_AND_INVITES.md) и [API.md](API.md).
